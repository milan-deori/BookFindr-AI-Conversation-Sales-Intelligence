...
const StudyPage = ({ user, setWishlist }) => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        if (res.ok) {
          const filtered = data
            .filter((book) => book.category === 'Study Book')
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
          setBooks(filtered);
        }
      } catch (err) {
        console.error('Error fetching study books:', err);
      }
    };
    fetchBooks();
  }, []);
  ...
  return (
    <div className="bg-[#f1f2f4] min-h-screen w-full max-w-screen-2xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-semibold text-gray-800">Study Books</h1>
      <hr className="my-2 border-gray-600 mb-4" />

      {books.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-20 text-gray-500">
          <FaBook className="text-7xl mb-6" />
          <p className="text-lg font-semibold">No study books available</p>
          <p className="mt-2 text-center max-w-sm">Please check back later or explore other categories.</p>
        </div>
      ) : (
